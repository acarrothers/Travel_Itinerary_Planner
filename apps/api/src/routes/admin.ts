...
// Resolve the caller's role. If no CHATR_API_KEYS are configured we run in dev mode
...
function resolveRole(req: FastifyRequest): Role | "dev" | null {
  const keys = process.env.CHATR_API_KEYS;
  if (!keys) return "dev";
